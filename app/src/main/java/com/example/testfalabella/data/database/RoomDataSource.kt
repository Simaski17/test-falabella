...
import com.example.testfalabella.data.toDomainindicator
...
class RoomDataSource(db: IndicatorDatabase) : LocalDataSource {
    ...
    override suspend fun findIndicatorByCode(code: String): List<Indicators> = withContext(Dispatchers.IO) {
        indicatorDao.findIndicatorByCode (code).map { it.toDomainindicator() }
    }
